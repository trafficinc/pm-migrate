...
class Dumper
{
    ...
    private function dumpVar(&$var, array $parents = [], int $level = 0, int $column = 0): string
    {
        if ($var === null) {
            ...
            throw new Exception('Cannot dump resource.');
    ...
    private function dumpArray(array &$var, array $parents, int $level, int $column): string
    {
        ...
        array_pop($parents);
        $wrap = strpos($outInline, "\n") !== false || $level * $this->indent_length + $column + strlen($outInline) + 3 > $this->wrapLength; // 3 = [],
    ...
    private function dumpObject(&$var, array $parents, int $level): string
    {
        if ($var instanceof Serializable) {
            ...
            throw new Exception('Cannot dump closure.');
        }

        $class = get_class($var);
        if ((new ReflectionObject($var))->isAnonymous()) {
            throw new Exception('Cannot dump anonymous class.');
...
//                }
        ...
        if ($args) {
            throw new Exception('Insufficient number of placeholders.');
        }
        return $res;
    }
